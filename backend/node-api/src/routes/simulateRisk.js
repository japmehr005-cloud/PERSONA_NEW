...
router.post('/check', async (req, res) => {
  ...
  const avgAmount = txns.length ? txns.reduce((s, t) => s + t.amount, 0) / txns.length : 0
  ...
  const riskPayload = {
    userId,
    actionType: req.body.actionType || 'SIMULATE',
  ...
  try {
    ...
    if (riskResult.decision === 'WARN' && !req.body.userConfirmed) {
    ...
    return res.json({ decision: riskResult.decision })
  } catch (err) {
